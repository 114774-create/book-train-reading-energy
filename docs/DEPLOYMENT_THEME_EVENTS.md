...
1. 命名為「布可列車獎勵日誌」（或任意名稱）

1. 建立名為 **Logs** 的工作表（Worksheet）
...
1. 在 Vercel Dashboard 中確認環境變數已設定

1. 合併 PR 到 main 分支，Vercel 會自動部署
...
1. 檢查 Google Sheets 中的 Logs 工作表，確認記錄已寫入
...
1. 驗證 `GOOGLE_SHEETS_CREDENTIALS` 環境變數是否為有效的 JSON 字串
...
1. 在 Supabase Dashboard 中驗證 `theme_events` 和 `books` 表是否存在
...
1. 確認 `requirements.txt` 中的所有依賴已安裝

1. 驗證環境變數在 Vercel Dashboard 中已設定
...
1. **使用 Vercel 環境變數**管理所有敏感資訊

1. **定期輪換 Google Service Account 金鑰**

1. **限制 Service Account 的權限**，只授予必要的 Google Sheets API 存取權

1. **監控 Google Sheets 的存取日誌**，確認沒有未授權的存取
...
1. **錯誤重試機制**：為 Google Sheets 寫入添加重試邏輯

1. **活動進度儀表板**：在前端顯示學生的活動進度

1. **活動統計報告**：生成每個活動的參與統計
